...
part 'pending_verification_event.dart';
part 'pending_verification_state.dart';

/// A bloc that manages the state of a PendingVerification according to
/// the event that is dispatched to it.
class PendingVerificationBloc
    extends Bloc<PendingVerificationEvent, PendingVerificationState> {
  final ApiService apiService;

  // Dependency injection melalui constructor
  PendingVerificationBloc({required this.apiService})
      : super(PendingVerificationState()) {
    on<FetchPendingVerificationData>(_onFetchPendingVerificationData);
  }

  Future<void> _onFetchPendingVerificationData(
    FetchPendingVerificationData event,
    Emitter<PendingVerificationState> emit,
  ) async {
    emit(state.copyWith(isLoading: true, error: null)); // Reset error state
    try {
      final url = Uri.parse('$baseUrl/orders/${event.pesananId}/detail');
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['success'] == true) {
          // Parsing data ke model PendingVerificationModel
          final model = PendingVerificationModel.fromJson(jsonResponse['data']);

          // Emit state dengan data yang berhasil diambil
          emit(state.copyWith(
            pendingVerificationModelObj: model,
            isLoading: false,
          ));
